...
<tr>  
        <td colspan="7">
            ...
            <select name="keyField">
                ...
                <option value="M_PHONE_NUM">전화번호</option>
            </select>
...
<%
	...
	pstmt = conn.prepareStatement("select * from manager0");
	rs = pstmt.executeQuery();
	
	if(keyword != null && !keyword.equals(""))
	{
		pstmt = conn.prepareStatement("select * from Manager0 where " + keyField.trim() + " LIKE '%" + keyword.trim()+"%'");
		rs = pstmt.executeQuery();
	}
	...
	out.println("<th scopre = \"cols\">사원번호</th> <th scopre = \"cols\">이름</th> <th scopre = \"cols\">주소 </th><th scopre = \"cols\"> 전화번호 </th>");
	out.println("</tr>");
	out.println("</thread>");
	while(rs.next())
	{
		out.println("<tr>");
		out.println("<td>"+rs.getString("EMP_NUM"));
		out.println("<td>"+rs.getString("M_NAME"));
		out.println("<td>"+rs.getString("ADDRESS"));
		out.println("<td>"+rs.getString("M_PHONE_NUM"));
		out.println("</tr>");
	}
	out.println("</table>");
